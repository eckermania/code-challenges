...
const listFoods = (recipe) => {
    ...
    recipe.ingredients.forEach((value, index) => {
      let locations = [];
      let myCurrentLine = recipe.ingredients[index];
  
      for (let i = 0; i< myCurrentLine.length; i++){
        if(myCurrentLine[i] === ' '){
          locations.push(i);
        }
      }
      let ingredient = myCurrentLine.slice(locations[1]+1);
...
const stepActions = (recipe) => {
  ...
  for(let i = 0; i <recipe.steps.length; i++) {
      let action = recipe.steps[i].split(' ')[0];
      result.push(action);
  }
  return result;
}
