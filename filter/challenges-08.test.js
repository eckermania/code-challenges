...
const filterStringsWithVowels = (input) => {
    let vowels = input.filter((val, idx) => {
      return input[idx].match(/(a|e|i|o|u)/)
  })
  return vowels
};
...
const notInFirstArray = (forbiddenValues, input) => {
  let unique = input.filter((val, idx) =>{
...
const getBaseStatGreaterThan = (input, minBaseStat) => {
    let statComparison = input.filter(val => val.baseStat > minBaseStat);
...
const getStatName = (input, minBaseStat) => {
    let statComparison = input.filter(val => val.baseStat > minBaseStat);
    let powers = statComparison.map(val => val.stat.name);
    return powers
};
...
const evenOddNumericValues = (input) => {
    let evenOdd = input.filter((val, idx)=> {
         return Number.isInteger(val)
    })
    
    let labels = evenOdd.map((val, idx) => {
      if (val % 2) {
        return 'odd'
      } else {
        return 'even'
